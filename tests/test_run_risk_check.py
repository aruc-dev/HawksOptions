...
class RunRiskCheckTests(unittest.TestCase):
    def _run_with_reconciled_pending_close(self, *, dry_run: bool):
        ...
        with (
            ...
            patch("scheduler.run_risk_check.write_greeks_snapshot", return_value=Path("snapshot.json")),
        ...
        return result, save_positions, mark_strategy_closed

    def test_dry_run_pending_close_reconciliation_does_not_persist(self):
        result, save_positions, mark_strategy_closed = self._run_with_reconciled_pending_close(dry_run=True)
    ...
    def test_reconciled_pending_close_updates_trade_log_even_when_new_closes_disabled(self):
        result, save_positions, mark_strategy_closed = self._run_with_reconciled_pending_close(dry_run=False)
